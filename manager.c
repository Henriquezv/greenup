...
char* tabela; //lista de strings de guests

char* get_mac_address_manager() {
...
void* discovery_service(void* arg) {
    ...
    char* mac_manager = get_mac_address_manager();
    char* hostname_manager = get_hostname_manager();
    char* manager_ip = inet_ntoa(discovery_addr.sin_addr);
    management_service(manager_ip, ntohs(discovery_addr.sin_port), mac_manager, hostname_manager, "unknown");
...
char* create_guest_list(struct guest_info* guest, int num_guests) {
    char* guest_list_str = (char*)malloc(num_guests * (20+16+18+10+11) * sizeof(char)); // max size of a guest info string
    if (guest_list_str == NULL) {
        fprintf(stderr, "Error: Failed to allocate memory for guest_list_str.\n");
        exit(1);
    }

    char* guest_info_str = (char*)malloc((20+16+18+10+11)*sizeof(char)); // max size of a guest info string
    if (guest_info_str == NULL) {
        fprintf(stderr, "Error: Failed to allocate memory for guest_info_str.\n");
        exit(1);
    }

    for (int i = 0; i < num_guests; i++) {
        sprintf(guest_info_str, "%s%s%s%s%d%s%s%s%s", guest->hostname, DIVISOR, guest->ip, DIVISOR, guest->id, DIVISOR, guest->status, DIVISOR, guest->mac_address);
        strcat(guest_list_str, guest_info_str);
        if (i < num_guests - 1) {
            strcat(guest_list_str, SEPARADOR);
        }
        guest = guest->next;
    }
...
void print_guest_list(struct guest_info* guest, int num_guests) {
    char* guest_list_str = create_guest_list(guest, num_guests);
    printf("%s\n", guest_list_str);
    free(guest_list_str);
}
    ...
    while (1) {
        ...
        while (curr_guest != NULL) {
            ...
            if (strcmp(buffer, "awaken") == 0) {
                ...
                //printf("\n  estou no sleep service quit lol\n ");
                ssize_t send_len = sendto(monitoring_socket, message, strlen(message), 0, (struct sockaddr*)&guest_addr, sizeof(guest_addr));
                if (send_len < 0) {
...
void management_service(char* ip, int port, char* mac, char* hostname, char* buffer) {
    ...
    tabela = create_guest_list(guest_list, num_guests);
    //const char* tabelinha = tabela;
    //print_guest_list(guest_list, num_guests);
    //getchar();
